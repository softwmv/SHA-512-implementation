...
void sha512(uint8_t* message, size_t len) {
    // Padding the message
    ...
    uint8_t* padded = (uint8_t*)calloc(new_len + 16, sizeof(uint8_t));
    if (!padded) {
    ...
    // Compute 128-bit bit length: high and low 64 bits
    uint64_t bit_len_low = ((uint64_t)len) << 3;
    uint64_t bit_len_high = ((sizeof(size_t) > 8 && len > 0x1FFFFFFFFFFFFFFF) ? ((uint64_t)(len >> 61)) : 0);
    
    // Write in big-endian
    for (int i = 0; i < 8; i++) {
        padded[new_len + i] = (bit_len_high >> (56 - 8 * i)) & 0xff;
    }
    for (int i = 0; i < 8; i++) {
        padded[new_len + 8 + i] = (bit_len_low >> (56 - 8 * i)) & 0xff;
    }
